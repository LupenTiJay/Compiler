...
public final class FRecursiveDescentParser implements Constants {
    ...
    Expr expr() { 
    	...
    	while (lexer.inspect("or")) {
  	      lexer.consume("or");
//  	      return expr();/
  	      tmp = new OrExpr(tmp, term());
//  	      return new OrExpr(tmp,term());
    	} 
...
//		      return factor();
    ...
    VarExpr var() { 
    	...
    	VarExpr a = new VarExpr(lexer.consumeID());
		 return a; 
    } // TODO // TODO: replace this stub
    ConstantExpr constant() { 
    	ConstantExpr tmp = null;
    	if(peekConstant()){
			lexer.consume("'");
			
			if(lexer.inspect("0")){
				tmp = ConstantExpr.FalseExpr;
			}
			else if(lexer.inspect("1")){
				tmp = ConstantExpr.TrueExpr;
			}
